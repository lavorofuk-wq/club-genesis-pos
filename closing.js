...
function clForm(date){
  if(!closingState.forms[date]){
    ...
    closingState.forms[date]={closedBy:"",staffWork:[],castWork:clSeedCastWork(day),expenses:[],allowances:[],actualCash:"",cashNote:""};
  }
...
function clAdd(section){const f=clForm(clDefaultDate());if(section==="staffWork")f.staffWork.push({staffId:"st_"+Date.now(),staffName:"",role:"other",startTime:"20:00",endTime:"01:00",breakMinutes:0});if(section==="expenses")f.expenses.push({category:"",amount:0,note:""});if(section==="allowances")f.allowances.push({type:"",amount:0,recipientId:"",recipientName:""});render();}
...
function clValidate(date,payload){
  ...
  payload.expenses.forEach(e=>{if(e.amount<0)errs.push("経費金額は0以上です");});
  payload.allowances.forEach(a=>{if(a.amount<0)errs.push("手当金額は0以上です");});
  return errs;
}
...
function rClosing(){
  ...
  html+='<div class="glass" style="border-radius:8px;padding:14px;margin-bottom:14px;"><div style="display:flex;justify-content:space-between;"><div class="st">手当</div><button class="btn" onclick="clAdd(\'allowances\')" style="padding:5px 10px;color:#4ade80;background:rgba(74,222,128,.08);border:1px solid rgba(74,222,128,.2);border-radius:4px;">追加</button></div>'+form.allowances.map((r,i)=>'<div style="display:grid;grid-template-columns:1fr .7fr 1fr auto;gap:6px;margin-bottom:8px;"><input class="ip" value="'+(r.type||"")+'" oninput="clInput(\'allowances\','+i+',\'type\',this.value)" placeholder="種類"/><input class="ip" type="number" value="'+clInt(r.amount)+'" oninput="clInput(\'allowances\','+i+',\'amount\',this.value)" placeholder="金額"/><input class="ip" value="'+(r.recipientName||"")+'" oninput="clInput(\'allowances\','+i+',\'recipientName\',this.value)" placeholder="対象者"/><button class="btn" onclick="clRemove(\'allowances\','+i+')" style="color:#ff6b6b;background:rgba(255,80,80,.1);border:1px solid rgba(255,80,80,.2);border-radius:4px;">×</button></div>').join("")+'</div>';
