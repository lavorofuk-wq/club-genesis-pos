...
function listAnalysisReportHtml(report,{print=false}={}){
  ...
  return '<article class="la-report">'
    ...
    +'<section class="la-block"><h3>場内延長</h3><div class="la-summary">'+metric("延長回数",report.extensionCount+'<em> 回</em>',"延長した来店卓数 "+report.extensionTables+" 卓")+metric("場内延長売上",money(report.extensionSales))+metric("場内指名に対する延長割合",ratio,"延長 "+report.extensionTables+" 卓 ÷ フリー卓の場内指名 "+report.extensionEligibleTables+" 卓")+'</div></section>'
    +(!print?'<aside class="la-notes"><p>回数：同じ来店テーブルは各種別1回。会計後の別のお客様は別の来店として数えます。フリー→場内指名はそれぞれ1回。再着席の時間は合算します。</p><p>表の平均：合計 ÷ 出勤日数。フリーの平均時間：同じ来店卓のフリー時間を合算し、5分を超える卓の合計時間 ÷ 対象卓数。5分以下の卓も表の回数・日平均には含みます。時間は分単位に四捨五入します。</p><p>場内率：場内指名についた来店卓数 ÷ フリーについた来店卓数。最初から場内指名の卓も含みます。フリーが0卓、またはフリー・場内指名の来店を特定できない場合は「—」で表示します。</p><p>場内延長：回数は延長操作数、割合は延長した来店卓数 ÷ フリー卓（本指名なし）で場内指名についた来店卓数。本指名のある来店は、延長の回数・売上・割合の分子と分母すべてから除外します。同じ来店で複数回延長しても割合の分子は1卓です。売上は売上情報と同じ配分（延長後の注文を含む小計）を使用します。分母0は「—」で表示します。</p><p>営業終了済みのデータのみ対象。対象時間は各営業日19:00〜翌18:59です。</p>'
