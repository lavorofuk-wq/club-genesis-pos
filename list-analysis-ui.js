...
function listAnalysisReportHtml(report){
  ...
  return '<article class="la-report">'
    ...
    +'<section class="la-block"><h3>接客実績 <small>表の平均は1出勤日あたり</small></h3><div class="la-table-scroll"><table class="la-table"><thead><tr><th scope="col">種別</th><th scope="col">回数</th><th scope="col">合計時間</th><th scope="col">平均回数 / 日</th><th scope="col">平均時間 / 日</th></tr></thead><tbody>'+rows+'</tbody></table></div><div class="la-free-average">'+metric("フリーの平均時間",listAnalysisDuration(report.freeAverage?.averageMs),"1来店卓あたり・合算5分以下を除外")+'</div></section>'
    ...
    +'<aside class="la-notes"><p>回数：同じ来店テーブルは各種別1回。会計後の別のお客様は別の来店として数えます。フリー→場内指名はそれぞれ1回。再着席の時間は合算します。</p><p>表の平均：合計 ÷ 出勤日数。フリーの平均時間：同じ来店卓のフリー時間を合算し、5分を超える卓の合計時間 ÷ 対象卓数。5分以下の卓も表の回数・合計時間には含みます。時間は分単位に四捨五入します。</p><p>場内延長：回数は延長操作数、割合は延長した来店卓数 ÷ 場内指名についた来店卓数。同じ来店で複数回延長しても割合の分子は1卓です。売上は売上情報と同じ配分（延長後の注文を含む小計）を使用し、本指名のあるテーブルは対象外です。分母0は「—」で表示します。</p><p>営業終了済みのデータのみ対象。対象時間は各営業日19:00〜翌18:59です。</p>'
